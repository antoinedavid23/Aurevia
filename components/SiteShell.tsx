...
export function Logo(){return <Link href="/" className="logo" aria-label="AUREVIA home"><span className="mark"><i/><b>≋</b></span><span>AUREVIA<small>PRIVATE CONCIERGE</small></span></Link>}

export function SiteShell({children}:{children:React.ReactNode}){
 const [open,setOpen]=useState(false); const [cookies,setCookies]=useState(true);
 return <><header><Logo/><nav>{nav.map(([n,h])=><Link key={h} href={h}>{n}</Link>)}<span className="lang">IT</span><Link className="button small" href="/valutazione">Valuta la tua proprietà</Link></nav><button className="menu-btn" aria-label="Apri menu" onClick={()=>setOpen(true)}><Menu/></button></header>
 {open&&<div className="mobile-menu"><button aria-label="Chiudi menu" onClick={()=>setOpen(false)}><X/></button><Logo/>{nav.map(([n,h])=><Link onClick={()=>setOpen(false)} key={h} href={h}>{n}</Link>)}<Link className="button" onClick={()=>setOpen(false)} href="/valutazione">Valuta la tua proprietà</Link></div>}
 <main>{children}</main>
 <footer><div className="footer-grid"><div><Logo/><p>L’arte di prendersi cura di ciò che conta.</p></div><div><b>Esplora</b>{nav.map(([n,h])=><Link key={h} href={h}>{n}</Link>)}</div><div><b>Proprietari</b><Link href="/proprietari">Gestione AUREVIA</Link><Link href="/valutazione">Valutazione privata</Link><Link href="/faq">Domande frequenti</Link></div><div><b>Contatti</b><a href={`mailto:${process.env.NEXT_PUBLIC_EMAIL||"info@aurevia.it"}`}>{process.env.NEXT_PUBLIC_EMAIL||"info@aurevia.it"}</a><span>Genova, Italia</span><div className="social"><span>IG</span><span>IN</span></div></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} AUREVIA</span><Link href="/privacy">Privacy</Link><Link href="/cookie-policy">Cookie</Link><Link href="/termini">Termini</Link></div></footer>
