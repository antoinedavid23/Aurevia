  ...
  return <>
    <header className={`site-header${headerHidden ? " is-hidden" : ""}`}>
      ...
      <div className="header-actions">
        ...
        <Link className="admin-login" href="/connexion" aria-label="Connexion" title="Connexion"><LogIn size={17}/><span className="sr-only">Connexion</span></Link>
      </div>
    ...
    <footer className="footer-premium">
      ...
      <div className="footer-grid">
        ...
        <div className="footer-contact"><b>Contact privé</b><a href={`mailto:${process.env.NEXT_PUBLIC_EMAIL || "contatto@aurevia-genova.com"}`}>{process.env.NEXT_PUBLIC_EMAIL || "contatto@aurevia-genova.com"}</a><a href={contactPhoneHref} data-no-translate>{contactPhone}</a><a href={instagramProfile.url} target="_blank" rel="noopener noreferrer" data-no-translate>Instagram · {instagramProfile.handle}</a><span>Gênes, Italie</span><p>Chaque demande est étudiée personnellement et traitée avec la plus grande discrétion.</p></div>
      </div>
